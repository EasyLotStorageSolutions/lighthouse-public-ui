...
export function mountMallMap({app,nav,go,explore,introduction,prefs}){
 ...
 const marquee=make('aside','lighthouse-digital-banner');marquee.setAttribute('aria-label','Lighthouse development notice');marquee.append(make('span','lighthouse-digital-status','LIVE UPDATE'),make('strong',null,'LIGHTHOUSE IS UNDER CONSTRUCTION'),make('span','lighthouse-digital-detail','New rooms and tools are opening soon. Check back often.'));
 ...
 world.append(canvas,panel,directory,theater);map.append(toolbar,marquee,world);const extras=app.querySelector('.harbor-discovery');if(extras)map.append(extras);map.append(conciergeSection);introduction.replaceWith(map);app.classList.add('mall-map-ready');
 ...
 const returnMap=btn('← Category Map','mall-return',()=>{go('home',true);showMap();map.scrollIntoView({block:'start'});});app.prepend(returnMap);import {openCenterPlayer} from './lighthouse-live.mjs?v=20260910-embedded-player1';
const districts=[
 {id:'marketplace',name:'Marketplace',hint:'Buy, sell, trade & discover',x:15,y:18,mx:20,my:10,angle:-145},
 {id:'employment',name:'Jobs & Hiring',hint:'Find work or hire people',x:38,y:12,mx:80,my:10,angle:-110},
 {id:'storage',name:'Storage & Space',hint:'Find or offer storage space',x:63,y:12,mx:20,my:28,angle:-70},
