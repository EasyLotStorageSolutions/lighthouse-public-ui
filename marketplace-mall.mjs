...
document.addEventListener('click',async event=>{
    ...
    try{switch(a){
        ...
        case'logout':{
            clearTimeout(saveTimer);if(draft)await saveDraft();
            const backupKey=draftStorageKey();await api('logout');
            writeLocal(backupKey,null);session={signedIn:false,owner:false};
            draft=null;draftRecord=null;draftStep=0;activeThread=null;records.clear();sellerStores=[];
            await showAccount();notify('Signed out. You can sign in with another account.');break;
        }
